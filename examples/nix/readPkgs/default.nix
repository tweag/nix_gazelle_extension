...
let
  ...
  readPkgs = { root }:
    let
      ...
      joinChildren = children: foldl' (acc: child: acc + "/${child}") root children;
      ...
      dirs =
        let
          ...
          all = attrNames (filterAttrs (n: v: v == "directory" && substring 0 1 n != ".") files);
          # Check if subdirectory should be ignored
          unignored = filter (d: ! pathExists (root + "/${d}/.nix-ignore-subdirectory")) all;
        in
        unignored;
